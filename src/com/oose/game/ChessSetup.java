...
public class ChessSetup extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		currentChess = intent.getIntExtra(ChessMainMenu.CHESSTYPE_INT,
				ChessMainMenu.ERROR);
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case PICTURERESULTCODE:
			...
			setup.setPlayerOnePicture((Bitmap) data
					.getParcelableExtra(PLAYER1ICON_BITMAP));
			setup.setPlayerTwoPicture((Bitmap) data
					.getParcelableExtra(PLAYER2ICON_BITMAP));
			break;
		case TIMERESULTCODE:
			setup.setTimeLimitValue(data.getIntExtra(TIMELIMIT_INT,
					DEFAULTIMELIMIT));
			setup.setTimeLimitSwitch(data.getBooleanExtra(LIMITSWITCH_BOOLEAN,
					true));
			break;
		default:
			break;
